...
impl Instruction {
    ...
    fn not_prefixed<T: Bus>(opcode: u8, address: u16, bus: &mut T) -> (Instruction, u16) {
        ...
        let instruction = match opcode {
            0x00 => NOP,
            0x01 => LD(Word(BC, WordSource::D16(read_u16(&mut address, bus)))),
            ...
            0x06 => LD(Byte(
                ByteTarget::R(B),
                ByteSource::D8(read_u8(&mut address, bus)),
            )),
            0x07 => RLCA,
            0x08 => LD(IndirectFromSP(ByteRef::D16(read_u16(&mut address, bus)))),
            ...
            0x0f => RRCA,
            0x10 => STOP,
            0x11 => LD(Word(DE, WordSource::D16(read_u16(&mut address, bus)))),
            ...
            0x17 => RLA,
            0x18 => JR(Always, read_u8(&mut address, bus) as i8),
            ...
            0x1f => RRA,
            0x20 => JR(NotZero, read_u8(&mut address, bus) as i8),
            0x21 => LD(Word(HL, WordSource::D16(read_u16(&mut address, bus)))),
            0x22 => LD(HLIFromAInc),
            0x23 => INC2(HL),
            ...
            0x27 => DAA,
            0x28 => JR(Zero, read_u8(&mut address, bus) as i8),
            ...
            0x2f => CPL,
            0x30 => JR(NotCarry, read_u8(&mut address, bus) as i8),
            0x31 => LD(Word(SP, WordSource::D16(read_u16(&mut address, bus)))),
            0x32 => LD(HLIFromADec),
            0x33 => INC2(SP),
            ...
            0x37 => SCF,
            0x38 => JR(Carry, read_u8(&mut address, bus) as i8),
            ...
            0xc0 => RET(NotZero),
            0xc1 => POP(BC),
            0xc2 => JP(NotZero, JumpTarget::D16(read_u16(&mut address, bus))),
            0xc3 => JP(Always, JumpTarget::D16(read_u16(&mut address, bus))),
            0xc4 => CALL(NotZero, read_u16(&mut address, bus)),
            0xc5 => PUSH(BC),
            0xc6 => ADD(ByteSource::D8(read_u8(&mut address, bus))),
            0xc7 => RST(RST00),
            0xc8 => RET(Zero),
            0xc9 => RET(Always),
            0xca => JP(Zero, JumpTarget::D16(read_u16(&mut address, bus))),
            0xcb => unreachable!("CB Prefix"),
            0xcc => CALL(Zero, read_u16(&mut address, bus)),
            0xcd => CALL(Always, read_u16(&mut address, bus)),
            0xce => ADC(ByteSource::D8(read_u8(&mut address, bus))),
            0xcf => RST(RST08),
            0xd0 => RET(NotCarry),
            0xd1 => POP(DE),
            0xd2 => JP(NotCarry, JumpTarget::D16(read_u16(&mut address, bus))),
            0xd3 => Illegal(0xd3),
            0xd4 => CALL(NotCarry, read_u16(&mut address, bus)),
            0xd5 => PUSH(DE),
            0xd6 => SUB(ByteSource::D8(read_u8(&mut address, bus))),
            0xd7 => RST(RST10),
            0xd8 => RET(Carry),
            0xd9 => RETI,
            0xda => JP(Carry, JumpTarget::D16(read_u16(&mut address, bus))),
            0xdb => Illegal(0xdb),
            0xdc => CALL(Carry, read_u16(&mut address, bus)),
            0xdd => Illegal(0xdd),
            0xde => SBC(ByteSource::D8(read_u8(&mut address, bus))),
            0xdf => RST(RST18),
            0xe0 => LD(IndirectFrom(
                ByteRef::D8(read_u8(&mut address, bus)),
                ByteSource::R(A),
            )),
            0xe1 => POP(HL),
            ...
            0xe4 => Illegal(0xe4),
            0xe5 => PUSH(HL),
            0xe6 => AND(ByteSource::D8(read_u8(&mut address, bus))),
            0xe7 => RST(RST20),
            0xe8 => ADDSP(read_u8(&mut address, bus) as i8),
            0xe9 => JP(Always, JumpTarget::HL),
            0xea => LD(IndirectFrom(
                ByteRef::D16(read_u16(&mut address, bus)),
                ByteSource::R(A),
            )),
            0xeb => Illegal(0xeb),
            0xec => Illegal(0xec),
            0xed => Illegal(0xed),
            0xee => XOR(ByteSource::D8(read_u8(&mut address, bus))),
            0xef => RST(RST28),
            0xf0 => LD(Byte(
                ByteTarget::R(A),
                ByteSource::I(ByteRef::D8(read_u8(&mut address, bus))),
            )),
            0xf1 => POP(AF),
            ...
            0xf4 => Illegal(0xf4),
            0xf5 => PUSH(AF),
            0xf6 => OR(ByteSource::D8(read_u8(&mut address, bus))),
            0xf7 => RST(RST30),
            0xf8 => LD(HLFromSPi8(read_u8(&mut address, bus) as i8)),
            0xf9 => LD(Word(SP, WordSource::R(HL))),
            0xfa => LD(Byte(
                ByteTarget::R(A),
                ByteSource::I(ByteRef::D16(read_u16(&mut address, bus))),
            )),
            0xfb => EI,
            0xfc => Illegal(0xfc),
            0xfd => Illegal(0xfd),
            0xfe => CP(ByteSource::D8(read_u8(&mut address, bus))),
            0xff => RST(RST38),
        };
        (instruction, address)
    }
...
impl fmt::Display for Instruction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let ident = match self {
            ...
            CALL(test, address) => match test {
                Always => format!("CALL {address:#06x}"),
                _ => format!("CALL {test}, {address:#06x}"),
            },
            ...
            SUB(source) => format!("SUB {source}"),
            SBC(source) => format!("SBC A, {source}"),
            AND(source) => format!("AND {source}"),
            OR(source) => format!("OR {source}"),
            XOR(source) => format!("XOR {source}"),
            CP(source) => format!("CP {source}"),
...
#[inline]
fn read_u8<T: Bus>(address: &mut u16, bus: &mut T) -> u8 {
...
/// increments the passed address and returns the read value.
fn read_u16<T: Bus>(address: &mut u16, bus: &mut T) -> u16 {
    let low = read_u8(address, bus);
    let high = read_u8(address, bus);

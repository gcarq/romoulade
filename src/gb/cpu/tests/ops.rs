...
fn test_word_register_af() {
    let mut cpu = CPU::default();
    cpu.r.set_af(0x1234);
    assert_eq!(AF.read(&mut cpu), 0x1230, "Lower 4 bits should be ignored");
...
fn test_word_register_bc() {
    let mut cpu = CPU::default();
    cpu.r.set_bc(0x1234);
    assert_eq!(BC.read(&mut cpu), 0x1234);
...
fn test_word_register_de() {
    let mut cpu = CPU::default();
    cpu.r.set_de(0x1234);
    assert_eq!(DE.read(&mut cpu), 0x1234);
...
fn test_word_register_hl() {
    let mut cpu = CPU::default();
    cpu.r.set_hl(0x1234);
    assert_eq!(HL.read(&mut cpu), 0x1234);
...
fn test_word_register_sp() {
    let mut cpu = CPU::default();
    cpu.sp = 0x1234;
    assert_eq!(SP.read(&mut cpu), 0x1234);
...
fn test_byte_ref_r() {
    ...
    assert_eq!(byte_ref.resolve(&mut cpu), 0xDEAD);
...
fn test_byte_ref_c() {
    ...
    assert_eq!(byte_ref.resolve(&mut cpu), 0xFF42);
...
fn test_byte_source_r() {
    ...
    assert_eq!(source.read(&mut cpu, &mut bus), 0x42);
...
fn test_byte_source_i() {
    ...
    assert_eq!(source.read(&mut cpu, &mut bus), 0x22);
...
fn test_word_source_r() {
    ...
    assert_eq!(source.read(&mut cpu), 0x1234);
...
fn test_jump_condition_not_zero() {
    ...
    assert_eq!(condition.resolve(&mut cpu), true);
    cpu.r.f.insert(FlagsRegister::ZERO);
    assert_eq!(condition.resolve(&mut cpu), false);
...
fn test_jump_target_d16() {
    ...
    assert_eq!(target.read(&mut cpu), 0x1234);

...
#[test]
fn test_dmg_boot_rom() {
    let mut rom = vec![0x00; 0x100];
    rom.extend([
        0x00, 0xc3, 0x50, 0x01, 0xce, 0xed, 0x66, 0x66, 0xcc, 0x0d, 0x00, 0x0b, 0x03, 0x73, 0x00,
        ...
        0xdc, 0x99, 0x9f, 0xbb, 0xb9, 0x33, 0x3e, 0x54, 0x45, 0x53, 0x54, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x04, 0x02, 0x01,
        0x01, 0x00, 0x9c, 0x18, 0x11,
    ]);
    ...
    bus.hwmode = HardwareMode::DMG;

    while bus.is_booting {
        cpu.step(&mut bus);
    }
...
#[test]
fn test_cgb_boot_rom() {
    let mut rom = vec![0x00; 0x100];
    rom.extend([
        0x00, 0xc3, 0x50, 0x01, 0xce, 0xed, 0x66, 0x66, 0xcc, 0x0d, 0x00, 0x0b, 0x03, 0x73, 0x00,
        0x83, 0x00, 0x0c, 0x00, 0x0d, 0x00, 0x08, 0x11, 0x1f, 0x88, 0x89, 0x00, 0x0e, 0xdc, 0xcc,
        0x6e, 0xe6, 0xdd, 0xdd, 0xd9, 0x99, 0xbb, 0xbb, 0x67, 0x63, 0x6e, 0x0e, 0xec, 0xcc, 0xdd,
        0xdc, 0x99, 0x9f, 0xbb, 0xb9, 0x33, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x54, 0x45, 0x53, 0x54,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0xe7, 0x19, 0x41,
    ]);
    ...
    assert_eq!(cpu.r.sp, 0xFFFE, "SP is invalid");
    assert_eq!(cpu.r.pc, 0x0100, "PC is invalid");
    assert_eq!(cpu.r.get_af(), 0x1180, "AF is invalid");
    assert_eq!(cpu.r.get_bc(), 0x0000, "BC is invalid");
    assert_eq!(cpu.r.get_de(), 0xFF56, "DE is invalid");
    assert_eq!(cpu.r.get_hl(), 0x000D, "HL is invalid");
    assert_eq!(cpu.ime, ImeState::Disabled, "IME should be disabled");
}

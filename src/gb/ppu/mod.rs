...
use crate::gb::ppu::misc::{Palette, Sprite};
...
impl PPU {
    ...
    /// Handles the OAMSearch mode and returns the new `LCDMode`.
    fn handle_oam_search(&mut self) -> LCDMode {
        ...
        let bg_address = self.r.lcd_control.bg_tile_map_area();
    ...
    /// Handles the HBlank mode and returns a tuple with the new `LCDMode` and
    /// whether a STAT interrupt should be requested.
    ...
    /// Handles the VBlank mode, requests an VBLANK interrupt
    /// and returns a tuple with the new LCDMode and whether a STAT interrupt should be requested.
    fn handle_vblank(&mut self, int_reg: &mut InterruptRegister) -> (LCDMode, bool) {
    ...
    /// Handles the PixelTransfer mode and returns a tuple with the
    /// new `LCDMode` and whether a STAT interrupt should be requested.
